...
function displayToday(data) {
  ...
  $("#dayInfo").children(".nameDateWeather").html(city + moment.unix(data.current.dt).format(" (M/DD/YYYY) "));
  ...
  $("#dayInfo").children(".temp").html("Temp: " + data.current.temp + "&deg;F");
  $("#dayInfo").children(".wind").html("Wind: " + data.current.wind_speed + " MPH");
  $("#dayInfo").children(".humid").html("Humidity: " + data.current.humidity + " &percnt;");
  $("#dayInfo").children(".uvi").html("UV Index: " + data.current.uvi);
}
...
console.log(`.fore${index + 1}`);
    $(`.fore${index + 1}`).children().empty();

    $(`.fore${index + 1}`).children(".date").html(moment.unix(data.daily[index].dt).format("M/DD/YYYY"));
    ...
    $(`.fore${index + 1}`).children(".temp").html("Temp: " + data.daily[index].temp.day + "&deg;F");
    $(`.fore${index + 1}`).children(".wind").html("Wind: " + data.daily[index].wind_speed + " MPH");
    $(`.fore${index + 1}`).children(".humid").html("Humidity: " + data.daily[index].humidity + " &percnt;");
    $(`.fore${index + 1}`).children(".uvi").html("UV Index: " + data.daily[index].uvi);
  }
}
